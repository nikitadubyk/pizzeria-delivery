...
import { IconHome, IconPizza, IconReceipt, IconSettings, IconUsers } from "@tabler/icons-react";
...
import { hasRestaurantPermission, RESTAURANT_PERMISSION as P, type RestaurantPermission } from "@/lib/auth/restaurant-permissions";
...
const menuItems: (AdminMenuItem & { permission: RestaurantPermission })[] = [
  { href: ROUTES.ADMIN.ROOT, permission: P.ADMIN_ACCESS, icon: IconHome, label: "Главная" },
  { href: ROUTES.ADMIN.MENU, permission: P.MENU_READ, icon: IconPizza, label: "Меню" },
  { href: ROUTES.ADMIN.ORDERS, permission: P.ORDERS_READ, icon: IconReceipt, label: "Заказы" },
  { href: ROUTES.ADMIN.SETTINGS, permission: P.SETTINGS_MANAGE, icon: IconSettings, label: "Настройки" },
  { href: ROUTES.ADMIN.EMPLOYEES, permission: P.EMPLOYEES_READ, icon: IconUsers, label: "Доступ сотрудников" },
];
...
export function RestaurantAdminShell({ children }: { children: ReactNode }) {
  ...
  return (
    <AdminShell
      ...
      menuItems={menuItems.filter(item => hasRestaurantPermission(user.role, item.permission))}
